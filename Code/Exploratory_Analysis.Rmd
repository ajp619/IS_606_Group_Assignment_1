...
for(stock in stock.range){
  ...
  loss <- sum((demand - stock)[demand - stock < 0] * details$cost[details$type == "ham"])
  # Try different things here: 25% off 50% off $1 off
  coupon.cost <- sum((demand - stock)[demand - stock > 0] * details$cost[details$type == "ham"] * .25)
  results[i] <- profit + loss - coupon.cost
  i = i + 1
}
